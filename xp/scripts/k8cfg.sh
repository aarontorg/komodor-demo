...
xk8s=( $(kubectl get xk8s.demo.komodor | grep -v NAME | awk '{print $1}') )
declare -a xk8s
for x in "${xk8s[@]}" ;
do
  print "xk8: $x"
  xk8sUid=$(kubectl get xk8s.demo.komodor $x -o jsonpath='{.metadata.uid}');
  print "xk8Uid: $xk8sUid"
  clusterName=$(kubectl get xk8s.demo.komodor $x -o jsonpath='{.spec.id}')
